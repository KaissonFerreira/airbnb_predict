...
# and `pip install airbnb_predict` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "airbnb_predict", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n airbnb_predict -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "airbnb_predict", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "airbnb_predict", "YOUR_CONSOLE_SCRIPT"]
